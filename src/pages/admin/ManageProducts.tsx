    ...
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 p-4 md:p-8">
        <div className="flex flex-col md:flex-row justify-between md:items-center mb-6 gap-4">
          ...
          <button onClick={openAddModal} className="w-full md:w-auto flex items-center justify-center bg-red-600 text-gray-900 font-bold py-2.5 px-5 rounded-lg hover:bg-red-700 transition">
            <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
            Agregar Producto
          </button>
        </div>

        <div className="mb-6 bg-gray-800 p-4 rounded-2xl border border-gray-800">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              ...
              <div className="relative">
                ...
                <input id="search-product" type="search" value={search} onChange={(e) => setSearch(e.target.value)} className="bg-gray-700 border border-gray-600 text-white text-sm rounded-lg focus:ring-red-500 focus:border-red-600 block w-full pl-10 p-2.5" placeholder="Buscar producto..." />
              </div>
            </div>
          </div>
          <div className="mt-4 flex flex-wrap gap-2">
            {designCategories.map(cat => (
              <button key={cat} onClick={() => setActiveCategory(cat === 'Todos' ? 'all' : cat)} className={`px-4 py-2 text-sm font-medium rounded-lg ${activeCategory === cat ? 'bg-red-600 text-black' : 'text-gray-300 bg-gray-700 hover:bg-gray-600'}`}>
                {cat}
              </button>
            ))}
          </div>
          ...
          {loading ? (
            ...
            visibleProducts.map(p => (
              <div
                ...
                className="product-card bg-gray-800 rounded-2xl border border-gray-800 overflow-hidden transform-gpu hover:scale-105 transition-transform duration-200 ease-out hover:shadow-xl cursor-pointer"
              >
                <div className="p-5">
                  <div className="flex justify-between items-start">
                    <h3 className="text-lg font-bold text-white">{p.name}</h3>
                    <span className="bg-red-500 text-gray-900 text-xs font-medium px-2.5 py-0.5 rounded-full">{p.category}</span>
                  </div>
                  {p.description && <p className="text-sm text-gray-400 mt-2">{p.description}</p>}
                  <div className="flex justify-between items-center mt-4">
                    <p className="text-xl font-semibold text-red-500">${p.price.toFixed(2)}</p>
        ...
        {modalMounted && (
          ...
          >
            <div className={`bg-gradient-to-b from-gray-900 via-gray-800 to-gray-800 rounded-2xl shadow-2xl w-full max-w-lg m-4 border border-gray-800 transition-all duration-300 ${modalActive ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
              <div className="p-6">
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                ...
                <form id="product-form" onSubmit={submitForm} className="mt-6 space-y-4">
                  <div>
                    ...
                    <input id="product-name" type="text" value={form.name} onChange={(e) => setForm(prev => ({ ...prev, name: e.target.value }))} className="bg-gray-700 border border-gray-600 text-white text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-60 block w-full p-2.5" placeholder="Ej: Margarita de Fresa" required />
                  </div>
                  <div>
                    <label htmlFor="product-description" className="block mb-2 text-sm font-medium text-gray-300">Descripción</label>
                    <div className="relative">
                      <textarea id="product-description" rows={3} value={form.description} onChange={(e) => setForm(prev => ({ ...prev, description: e.target.value }))} className="bg-gray-700 border border-gray-600 text-white text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-60 block w-full p-2.5" placeholder="Describe el producto..." />
                    </div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                      <label htmlFor="product-category" className="block mb-2 text-sm font-medium text-gray-300">Categoría</label>
                      <select id="product-category" value={form.category} onChange={(e) => setForm(prev => ({ ...prev, category: e.target.value as any }))} className="bg-gray-700 border border-gray-600 text-white text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-60 block w-full p-2.5">
                    ...
                    <div>
                      ...
                      <input id="product-price" type="number" value={form.price === 0 ? '' : form.price} onChange={(e) => setForm(prev => ({ ...prev, price: Number(e.target.value) }))} className="bg-gray-700 border border-gray-600 text-white text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-60 block w-full p-2.5" placeholder="150.00" required />
                    </div>
